...
export default function Contact() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    const msg = `Hello The Baking Lab!\n\n** Subject:** ${form.subject}\n** Name:** ${form.name}\n** WhatsApp/Phone:** ${form.phone}\n** Email:** ${form.email || 'Not provided'}\n** Message:** ${form.message}`
    window.open(`https://wa.me/9779845126192?text=${encodeURIComponent(msg)}`, '_blank')
    setSent(true)
  ...
  return (
    <div className="bg-[#FAF0E0] dark:bg-[#1C0F0A] min-h-screen">
      <Helmet>
        <title>Contact | The Baking Lab — Bakery in Kaldhara, Thamel, Kathmandu</title>
        <meta name="description" content="Visit The Baking Lab at Kaldhara Chowk, Thamel, Kathmandu. Call or WhatsApp +977-984-5126192 to pre-order banana bread, custom cakes & eggless bakes. Open Mon-Sat 7AM-7PM." />
        <link rel="canonical" href="https://thebakinglab.com.np/contact" />
        <meta property="og:title" content="Contact | The Baking Lab — Bakery in Thamel, Kathmandu" />
        <meta property="og:description" content="Visit The Baking Lab at Kaldhara Chowk, Thamel. Call/WhatsApp to pre-order banana bread, custom cakes & eggless bakes." />
      </Helmet>
      {/* Hero */}
      <section
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <motion.div {...fadeUp} className="bg-[#2C1506] rounded-2xl p-8 shadow-xl text-[#FDF6EC] space-y-6">
            ...
            {/* Follow Us */}
            <div className="border-t border-[#FDF6EC]/10 pt-5">
              <p className="text-[#D4A017] text-sm uppercase tracking-widest mb-4">Follow Us</p>
              <div className="flex gap-4">
                <a href="https://www.facebook.com/ThebakinglabNP" target="_blank" rel="noopener noreferrer" className="group relative w-10 h-10 border border-[#D4A017] rounded-full flex items-center justify-center text-[#D4A017] hover:bg-[#D4A017] hover:text-[#3B1A0A] transition duration-200">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
                  <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-[#3B1A0A] text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Facebook</span>
                </a>
                <a href="https://www.instagram.com/bakinglabaadhunik" target="_blank" rel="noopener noreferrer" className="group relative w-10 h-10 border border-[#D4A017] rounded-full flex items-center justify-center text-[#D4A017] hover:bg-[#D4A017] hover:text-[#3B1A0A] transition duration-200">
                  ...
                  <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-[#3B1A0A] text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Instagram</span>
                </a>
                <a href="https://www.tiktok.com/@the.baking.lab" target="_blank" rel="noopener noreferrer" className="group relative w-10 h-10 border border-[#D4A017] rounded-full flex items-center justify-center text-[#D4A017] hover:bg-[#D4A017] hover:text-[#3B1A0A] transition duration-200">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" /></svg>
                  <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-[#3B1A0A] text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">TikTok</span>
                </a>
              </div>
            </div>
